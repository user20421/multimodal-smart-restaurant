...
```bash
# 前端：构建产物到 frontend/dist/（Vite 默认已压缩混淆）
cd frontend
npm run build

# 打成 zip（Windows PowerShell），得到 frontend/dist.zip
Compress-Archive -Path dist -DestinationPath dist.zip -Force
```

后端无需构建，直接把 `backend/` 目录打成 `backend.zip` 上传即可（应排除 `__pycache__/`、`.pytest_cache/`；包内含 `requirements.txt` 与 `.env` 模板）。`backend.zip` 与 `frontend/dist.zip` 均已加入 `.gitignore`，不会入库。

服务器上解压后：

```bash
pip install -r requirements.txt          # 安装后端依赖
# 编辑 backend/.env：IS_SERVER=true，填入真实 JWT 密钥、两个大模型 API Key、数据库连接 URL
mysql -u root -p < scripts/init.sql      # 初始化数据库（此脚本在仓库 scripts/ 下，不在 backend.zip 内，需单独上传）
uvicorn app.main:app --host 0.0.0.0 --port 8000
```

前端 `dist/` 交给 Nginx 等静态服务器托管（指向 `index.html`）即可。
...
- **聊天次数配额**：普通用户初始 30 次发送额度，耗尽弹窗提示“次数不足，请联系开发人员”；超管面板可查看剩余次数并充值（+100/次）
