...
# 清空购物车
_CLEAR_PATTERNS = [
    re.compile(r"^(?:请|麻烦)?(?:帮我|给我)?(?:清空|清除|清空一下|清除一下)(?:我的)?购物车$"),
    re.compile(r"^(?:把)?(?:我的)?购物车(?:给)?(?:清空|清除|清空一下|清除一下)$"),
]

# 查看购物车
_VIEW_CART_PATTERN = re.compile(
    r"^(?:查看|看看|看下|看一下|打开)?(?:我的)?购物车(?:里)?"
    r"(?:有什么|有啥|有什么菜|有啥菜|里有什么|里有什么菜)?$"
)

# 下单 / 结算
_PLACE_ORDER_PATTERN = re.compile(
    r"^(?:就这些|好了|可以了)?[,，]?(?:请|麻烦)?(?:帮我|给我)?"
    r"(?:确认下单|提交订单|结算|买单|下单)$"
)
...
_MY_ORDERS_PATTERN = re.compile(r"^(?:查看|看看|看下|看一下)?(?:我的)?订单$")

# 换菜：把X换成Y / 把两份X换成一份Y / 把其中两份X换成一份Y（部分换）
_SWAP_PATTERN = re.compile(r"^(?:把)?(.+?)(?:换成|换为)(.+?)$")
# 改数量：X改成2份 / X调整为3份
_SET_TO_PATTERN = re.compile(rf"^(?:把)?(.+?)(?:改成|调整为){_QTY_RE}{_UNIT}?$")
# 只要 N 份：宫保鸡丁只要一份
_SET_QTY_PATTERN = re.compile(rf"^(.+?)(?:只要|只需要|就留|留){_QTY_RE}{_UNIT}?$")

# 减/删菜
_REMOVE_PATTERNS = [
    re.compile(r"^(?:把)?(.+?)(?:给我)?(?:去掉|删掉|删除|拿掉|移除|不要了|不要)$"),
    re.compile(r"^(?:去掉|删掉|删除|拿掉|移除|不要)(.+?)$"),
]

# 加菜：来三份宫保鸡丁 / 我要一份米饭 / 再加两个烤串 / 点一份汤
_ADD_PATTERN = re.compile(
    rf"^(?:请|麻烦)?(?:我|我们)?(?:帮我|给我|给我们)?(?:来|要|加|再加|多点|点|上|上一份)"
    rf"{_QTY_RE}?{_UNIT}?(.+?)$"
)
...
async def try_handle(ctx: AgentContext, message: str) -> Optional[str]:
    ...
    # 2. 查看购物车
    if _VIEW_CART_PATTERN.match(text):
