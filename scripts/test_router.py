...
的划分是否符合设计阈值：
- 同类多操作 ≤3 件且简单 -> 留在专员（cart/order）
- 单件复杂操作能理解 -> 专员；理解不了 -> manager
- 跨类别混合、每件事都明确 -> manager
...
CASES = [
    ...
    ("来一份麻婆豆腐、两份宫保鸡丁，再加一碗米饭", {"cart"}, "同类3件加菜"),
    ("把宫保鸡丁改成三份，再把麻婆豆腐移除", {"cart"}, "同类2件改+删"),
    # ---- 单件复杂但能理解 -> 专员 ----
    ("把其中两份宫保鸡丁换成一份麻婆豆腐", {"cart"}, "部分换菜"),
    # ---- 同类 4 件（边界：专员或经理均可接受，不应 unclear）----
    ("麻婆豆腐、宫保鸡丁、鱼香肉丝、水煮鱼各来一份", {"cart", "manager"}, "同类4件边界"),
    # ---- 跨类别混合 -> 经理 ----
    ("你们今天的营业时间是什么，帮我来一份麻婆豆腐和2份宫保鸡丁，再来一份白米饭",
     {"manager"}, "咨询+加3样（用户原始用例）"),
...
REPEAT_FOR = {"同类4件边界", "咨询+加3样（用户原始用例）", "查单+加菜", "条件咨询+加菜",
              "查单+清空", "过多过杂+矛盾"}
REPEAT_TIMES = 2
